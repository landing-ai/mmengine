...
class TestCheckpointHook(RunnerTestCase):
    ...
    def test_with_runner(self, training_type):
        ...
        runner = self.build_runner(cfg)
        runner.train()
        ckpt = torch.load(osp.join(cfg.work_dir, f'{training_type}_11.pth'), weights_only=True)
        ...
        ckpt = torch.load(osp.join(cfg.work_dir, f'{training_type}_5.pth'), weights_only=True)
