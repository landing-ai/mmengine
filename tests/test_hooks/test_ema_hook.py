...
class TestEMAHook(RunnerTestCase):
    ...
    def test_with_runner(self):
        ...
        checkpoint = torch.load(osp.join(self.temp_dir.name, 'epoch_2.pth'), weights_only=True)
        ...
        state_dict = torch.load(
            osp.join(self.temp_dir.name, 'epoch_4.pth'), map_location='cpu', weights_only=True)
        ...
        runner.train()
        state_dict = torch.load(
            osp.join(self.temp_dir.name, 'iter_4.pth'), map_location='cpu', weights_only=True)
        self.assertIn('ema_state_dict', state_dict)
        for k, v in state_dict['state_dict'].items():
            assert_allclose(v, state_dict['ema_state_dict']['module.' + k])
        state_dict = torch.load(
            osp.join(self.temp_dir.name, 'iter_5.pth'), map_location='cpu', weights_only=True)
        self.assertIn('ema_state_dict', state_dict)
