...
from mmengine.logging import print_log, HistoryBuffer
...
# allowlist these globals so that it can be loaded from torch.load
...
def _get_mmengine_home():
    mmengine_home = os.path.expanduser(
        os.getenv(
            ENV_MMENGINE_HOME,
            os.path.join(os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), "mmengine"),
        )
    )
...
def load_from_http(filename, map_location=None, model_dir=None, progress=os.isatty(0)):
    ...
    if rank == 0:
        checkpoint = load_url(
            filename, model_dir=model_dir, map_location=map_location, progress=progress
        )
    if world_size > 1:
...
def weights_to_cpu(state_dict):
    ...
    metadata = getattr(state_dict, "_metadata", OrderedDict())
    state_dict = apply_to(state_dict, lambda x: hasattr(x, "cpu"), lambda x: x.cpu())
    state_dict._metadata = metadata
    return state_dict
